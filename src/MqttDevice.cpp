                       ...
                       const char *willTopic, const char *willPayload, bool willRetain, int willQos, int mqttKeepAliveSeconds)
{
  ...
  log("Begin done: wifi SSID='%s', mqtt='%s:%d'", _ssid.c_str(), _mqttHost.c_str(), _mqttPort);
}
...
void MqttDevice::handleMessage(const char *topic, byte *payload, unsigned int length)
{
  ...
  // Callbacks Binárias (Prioridade Máxima)
  ...
  // Callbacks JSON (incluindo Request/Response)
  bool anyJson = false;
                         ...
                         unsigned long timeoutMs)
{
  ...
  // Clona e modifica o documento de pedido
...
void MqttDevice::checkPendingRequests()
{
  ...
  for (auto it = pendingRequests.begin(); it != pendingRequests.end();)
  {
    ...
    {
      // Log de timeout e limpeza de callback, se necessário
...
void MqttDevice::loop()
{
  ...
  // Gestão de recursos (limpeza de pedidos/timeouts)
